...
// * Initiator flow *
// ******************
public class BankApproveFlow{

@InitiatingFlow
@StartableByRPC
    public static class BankApproveInitiate extends FlowLogic<SignedTransaction> {

    private final UniqueIdentifier trackingid;
    private final Party owner;

    public BankApproveInitiate(UniqueIdentifier trackingid, Party owner) {
    ...
    public SignedTransaction call() throws FlowException {
        ...
        if (statetransfer.getOwner().equals("") || statetransfer.getBuyerId()==0 || statetransfer.getSellerId()==0 || statetransfer.isSurveyorApproved() == false)
        {
            System.out.println("Inside If condition");
            tx.addOutputState(statetransfer.approveByBank(false, getOurIdentity()).transfer(owner, getOurIdentity()));
        }
        else{
            System.out.println("Inside Else condition");
            tx.addOutputState(statetransfer.approveByBank(true, getOurIdentity()).transfer(owner, getOurIdentity()));
        }
    ...
    @InitiatedBy(BankApproveInitiate.class)
    public static class BankApproveRespond extends FlowLogic<SignedTransaction> {

        private final FlowSession countersession;

        public BankApproveRespond(FlowSession countersession) {
